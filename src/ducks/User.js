...
export default (state = INITIAL_STATE, action) => {
  ...
  switch (action.type) {
    ...
    case ACTIONS.GO_TO_REGISTRATION:
      return INITIAL_STATE;
    // Covers ACTIONS.GO_TO_PROFILE, ACTIONS.GO_TO_EDIT_PROFILE_FORM
    default:
      return state;
  }
};
  ...
  return dispatch => {
    if (currentUser != null) {
      firebase
        ...
        .ref(`/users/${id}/`)
        .on("value", snapshot => {
          dispatch({ type: ACTIONS.LOAD_USER, payload: snapshot.val() });
          dispatch({ type: ACTIONS.PAGE_LOAD, payload: false });
        });
    }
  };
};
