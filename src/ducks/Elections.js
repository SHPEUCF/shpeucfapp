...
export default (state = INITIAL_STATE, action) => {
  ...
  switch (action.type) {
    ...
    // Also covers ACTIONS.APPROVE_APPLICATION, ACTIONS.ADD_APPLICATION, ACTIONS.EDIT_POSITION, and EDIT_CANDIDATES
    default:
      return state;
  }
};
...
export const editPosition = (title, description, oldTitle) => {
  if (oldTitle !== null) {
    ...
    firebase
      .database()
      .ref(`/election/positions/${oldTitle}/level`)
      .once("value", snapshot => {
        level = snapshot.val();
      });
...
export const addApplication = (fName, lName, plans, position, picture) => {
  ...
  return dispatch => {
    firebase
      .database()
      .ref(`/election/positions/${position}/candidates/${uid}`)
      .set({
        firstName: fName,
        lastName: lName,
        plan: plans,
...
export const editApplication = (plans, position, candidateId) => {
  ...
  firebase
    .database()
    .ref(`/election/positions/${position}/candidates/${uid}/plan`)
    .set(plans)
  ...
  return dispatch => {
    ...
    firebase
      ...
      .then(() => {
        firebase
          .database()
          .ref(`/voting/${position}/${candidateId}`)
          .set({
            votes: 0,
            firstName: fName,
            lastName: lName
      ...
      .then(() => {
        dispatch({ type: ACTIONS.APPROVE_APPLICATION });
      })
