...
class ElectionApplication extends Component {
  ...
  showListPosition(positionsArray) {
    if (this.state.isListShow == false) return null;
  ...
  renderListPositionComponent(item) {
    const { button, textStyle, textColor } = styles;
        ...
        <View style={button}>
          <Button
            ...
            onPress={() => {
              ...
              this.setState({ isApplyShow: true });
              this.setState({ applyPos: item.title });
            }}
          />
        </View>
      </View>
  ...
  renderButtons() {
    const { firstName, lastName, candidatePlan, id, addApplication, buttonContainer, picture } = this.props;
    const { application, applyPos, isListShow } = this.state;
