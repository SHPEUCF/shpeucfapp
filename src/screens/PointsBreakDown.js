...
class PointsBreakDown extends Component {
  ...
  renderContent() {
    ...
    const { containerStyle, contentContainerStyle, title, points, topLevelText, textColor } = styles;
    ...
    if (membersPoints !== undefined && membersPoints !== null && membersPoints[currentUser.uid] !== undefined &&
      ...
      return (
        <View style={{ flex: 1 }}>
          <View style={[contentContainerStyle, containerStyle]}>
            ...
            <Text style={[points, topLevelText, textColor]}>
              {this.props.membersPoints[currentUser.uid].points}
            </Text>
          </View>
          <FlatList
            data={breakdown}
      ...
      return <Text style={[textColor, { flex: 1 }]}>You have no Points! Go out there and get some points!</Text>;
  }

  _keyExtractor = (item, index) => item;

  renderComponent(section) {
    const { containerStyle, contentContainerStyle, title, points, midLevelText, textColor } = styles;
    // alert(item[0])
    ...
    return (
      <View>
        ...
        <FlatList
          ...
          extraData={this.state}
          keyExtractor={this._keyExtractor}
          renderItem={({ item, separators }) => this.renderInnerComponent(item, section[0])}
        />
      </View>
    );
  }
  ...
  renderInnerComponent(item, section) {
    const { innerContainerStyle, innerContentContainerStyle, title, points, botLevelText, textColor } = styles;
...
const mapDispatchToProps = { fetchMembersPoints, loadUser, fetchEvents, goToViewEvent };
