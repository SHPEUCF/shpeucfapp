...
class ElectionBackEnd extends Component {
  ...
  render() {
    ...
    const { votes, positions } = this.props;
    let positionOrder = [];
    ...
    return (
      <View style={page}>
        ...
        <View style={{ flex: 20 }}>{this.renderFlatlist(positionOrder)}</View>

        <View style={buttonContainerStyling}>{this.openOrClose()}</View>
        <View style={buttonContainerStyling}>{this.applyOpenOrClose()}</View>
        <View style={buttonContainerStyling}>
          <Button onPress={() => Actions.ElectionPositions()} title={"MANAGE POSITIONS"}></Button>
        </View>
        <View style={buttonContainerStyling}>
          <Button onPress={() => Actions.ElectionCandidates()} title={"MANAGE CANDIDATES"}></Button>
        </View>
      </View>
    );
  }
  ...
  openOrClose() {
    if (this.props.election) {
      return <Button onPress={() => this.props.closeElection()} title={"CLOSE ELECTION"}></Button>;
    } else
      return <Button onPress={() => this.props.openElection()} title={"OPEN ELECTION"}></Button>;
  }
    ...
    if (this.props.apply) {
      return (
        <Button
          ...
          title={"CLOSE APPLICATIONS"}></Button>
      );
    } else
      return (
        <Button onPress={() => this.props.openApplications()} title={"OPEN APPLICATIONS"}></Button>
      );
  }
  ...
  renderVotes(item, index) {
    ...
    while (
      i + 1 < candidatesArray.length &&
      candidatesArray[i].votes === candidatesArray[i + 1].votes
    ) {
