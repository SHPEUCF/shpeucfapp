...
class Dashboard extends Component {
  ...
  renderContent() {
    ...
    sortedMembers.forEach((x, index) => {
      ...
      currentMember = x.id === currentUser.uid ? x : currentMember;
      pastPoints = x.points;
      pastIndex = x.index;
    });
    sortedMembers.splice(2);

    if (this.isDefined(currentMember) && this.isDefined(sortedMembers) && 
        sortedMembers[0].id !== currentMember.id && sortedMembers[1].id !== currentMember.id)
  ...
  convertNumToDate(date) {
    ...
    return `${months[Number(temp_date[1]) - 1].substr(0, 3)} ${temp_date[2]}, ${temp_date[0]}`;
  }
    ...
    return (
      <View>
        <Text style={[textColor, { fontSize: 20 }]}>
          {greeting}, {this.props.firstName}.
        </Text>
        <Text style={textColor}>
          Today is {months[month]} {day}
        </Text>
      </View>
    );
  }
