...
class ElectionCandidates extends Component {
  ...
  render() {
    ...
    return (
      <View style={page}>
        ...
        <View style={content}>{this.renderFlatlist(positionsArray)}</View>
      </View>
    );
  }
    ...
    return (
      <View>
        ...
        <FlatList
          ...
          renderItem={({ item, separators }) => this.renderCandidates(item)}
        />
      </View>
    );
  }
  ...
  renderCandidates(item) {
    const { textColor, containerTextStyle } = styles;

    const color = item.approved
      ? { backgroundColor: "#ffd70088" }
      : { backgroundColor: "#2C323988" };
  ...
  renderbuttons(item) {
    const { position, id, firstName, lastName } = item;
    const { deleteApplication, approveApplication } = this.props;
  ...
  renderFlatlist(positions) {
    return (
      <FlatList
        ...
        extraData={this.state}
        keyExtractor={this._keyExtractor}
        renderItem={({ item, separators }) => this.renderPositions(item)}
      />
    );
  }
}
