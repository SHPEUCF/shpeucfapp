...
class OtherProfile extends Component {
  render() {
    ...
    return this.props.loading ? <Spinner>{this.renderContent}</Spinner> : <View>{this.renderContent()}</View>;
  }
        ...
        <View style={{ backgroundColor: "#0c0b0b" }}>
          <View style={contentContainerStyle}>
            <View style={contentItemsContainerStyle}>
              <View style={{ height: dimension.height * 0.1, backgroundColor: "#0c0b0b" }}>
          ...
          <View style={buttonsContainerStyle}>{this.renderButtons()}</View>
        </View>
      </ScrollView>
    );
  }
  ...
  renderEditButton(privilege) {
    if (privilege.board)
      return (
        <View>
          <Button title="EDIT PROFILE" onPress={() => goToEditOtherProfileForm()} />
        </View>
      );
  }

  renderButtons() {
    const { privilege, goToEditOtherProfileForm } = this.props;

    return (
      <View>
        {this.renderEditButton(privilege)}
        <View>
          <Button title="BACK TO LEADERBOARD" onPress={() => Actions.popTo("Leaderboard")} />
        </View>
      </View>
    );
  }
  ...
  renderSocialMedia() {
    return (
      <View style={styles.editLogoContainer}>
        ...
        <View style={styles.editLogoContainer}>
          <TouchableOpacity onPress={() => Actions.PostShow({ title: "Github", uri: "https://www.github.com/" })}>
...
const mapDispatchToProps = { goToEditOtherProfileForm };
