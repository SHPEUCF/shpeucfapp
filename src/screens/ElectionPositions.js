...
class ElectionPosition extends Component {
  ...
  render() {
    const { buttonContainerStyling, page } = styles;
      ...
      <View style={page}>
        ...
        <View style={buttonContainerStyling}>
          <Button
            ...
            title={"ADD POSITIONS"}></Button>
        </View>
        <View style={buttonContainerStyling}>
          <Button onPress={() => this.setLevels()} title={"SET ORDER"}></Button>
        </View>
      </View>
    );
  }
  ...
  renderPositions({ item, index, move, moveEnd, isActive }) {
    ...
    return (
      <TouchableOpacity style={[contentContainerStyle, color]} onLongPress={move} onPressOut={moveEnd}>
  ...
  delete(position) {
    this.setState({ data: this.state.data.filter(item => item.position !== position) });
