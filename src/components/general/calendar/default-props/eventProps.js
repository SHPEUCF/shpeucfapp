...
export const DefaultItem = ({ item }) => {
	...
	return (
		<TouchableOpacity onPress = { () => viewEvent(item, dispatch) }>
			<View style = { [itemContainer, { backgroundColor: state.user.dashColor }] }>
...
export const DefaultEmptyData = () => {
	...
	const color = useSelector(state => state.user.dashColor);
...
const viewEvent = (item, dispatch) => {
	// dispatch(typeChanged(item.type));
	// dispatch(committeeChanged(item.committee));
	// dispatch(nameChanged(item.name));
	// dispatch(descriptionChanged(item.description));
	// dispatch(dateChanged(item.date));
	// dispatch(startTimeChanged(item.startTime));
	// dispatch(endTimeChanged(item.endTime));
	// dispatch(locationChanged(item.location));
	// dispatch(epointsChanged(item.points));
	// dispatch(eventIDChanged(item.eventID));
	// dispatch(goToViewEvent("events"));
};
