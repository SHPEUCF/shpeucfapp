...
class SortableFlatList extends Component {
  ...
  getSortedList = (data, activeRow, spacerIndex) => {
    ...
    const sortedData = data.reduce((acc, cur, i, arr) => {
      ...
      else if (i === spacerIndex) acc = [...acc, arr[activeRow], cur];
      else acc.push(cur);
      return acc;
    }, []);
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (prevProps.extraData !== this.props.extraData) this.setState({ extraData: this.props.extraData });
  };
...
SortableFlatList.defaultProps = { scrollPercent: 5, scrollSpeed: 5, contentContainerStyle: {} };
