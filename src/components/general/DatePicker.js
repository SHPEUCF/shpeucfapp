...
class DatePicker extends Component {
  constructor(props) {
    ...
    let date = [];
    if (this.props.value !== undefined && this.props.value !== null) date = this.props.value.split("-");
  ...
  render = () => {
    ...
    const { month, day, year, monthArr, dayArr, yearArr, focused } = this.state;
    let iconSize = 32;

    if (!focused)
      return (
        <View>
          <Input placeholder={placeholder} onFocus={() => this.setState({ focused: true })} />
        </View>
      );
    else
      return (
