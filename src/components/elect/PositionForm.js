...
class PositionForm extends Component {
  ...
  onButtonPress() {
    const { positionTitle, candidatePlan, positionDescription, positions } = this.props;

    let length = positions !== null && positions !== undefined ? Object.entries(positions).length : 0;
    ...
    if (positionTitle === "") {
      ...
      if (this.props.title === "ADD") this.props.addPosition(positionTitle, positionDescription, length);
      else {
  ...
  render() {
    return (
      <View style={styles.formContainerStyle}>
        ...
        <Button title={this.props.title + " POSITION"} onPress={this.onButtonPress.bind(this)} />
        <Button title="CANCEL" onPress={Actions.ElectionPositions.bind(this)} />
      </View>
    );
  }
}
