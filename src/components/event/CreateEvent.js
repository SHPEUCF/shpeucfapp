...
class CreateEvent extends Component {
	...
	onButtonPress() {
		...
		let startTimeComp = startTime.split(":");
		let endTimeComp = endTime.split(":");

		if (!this.state.changedStart && this.props.title === "Edit Event") {
			let newS = this.convertToMilitary({ hour: startTimeComp[0], minute: startTimeComp[1], period: startTimeComp[2] });
			startTimeComp = newS.split(":");
		}
		if (!this.state.changedEnd && this.props.title === "Edit Event") {
			let newE = this.convertToMilitary({ hour: endTimeComp[0], minute: endTimeComp[1], period: endTimeComp[2] });
	...
	renderComponent(item) {
		...
		let last = item[0] == dataArr[dataArr.length - 1]
		...
		showCommittees() {
			...
			if (this.props.type == "Committee" && this.props.committeesList)
				return (
					<View>
						<Modal
							transparent = { true }
			...
			return (
				...
				>
					<SafeAreaView style = { styles.formContainerStyle }>
						<View style = {{ backgroundColor: "black", flex: 1 }}>
							...
							<ScrollView
								...
								<View>
									...
									<Input
										placeholder = "Value"
										value = { this.props.points === 0 || !this.props.points ? "" : this.props.points.toString() }
						...
						<View style = {{ height: dimension.height * 0.08, backgroundColor: "#0c0b0b" }}></View>
					</SafeAreaView>
				</KeyboardAwareScrollView>
			);
		}
