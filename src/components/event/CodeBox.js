...
const CodeBox = ({
  ...
  const { modalContent, input, text, container, modal } = styles;
  return (
    <View style={[modal]} visible={visible}>
      <Modal
        ...
        <View style={[container]}>
          ...
          <TextInput
            ...
            maxLength={4}
          />
          <Button title="CHECK IN" width={dimension.width * 0.6} onPress={(code, eventID, points) => {}} />
        </View>
      </Modal>
    </View>
  );
...
CodeBox.defaultProps = {
  ...
  onPress: function(code, eventID, points) {
    if (code === this.state.text) checkIn(eventID, points, null);
  }
};
