...
const collegeNames = [];
collegesJson.map(college => {
  collegeNames.push(college.collegeName);
});

let majorNames = {};
collegesJson.map(college => {
  majorNames[college.collegeName] = college.degrees;
});

const continents = Object.keys(countriesJson);
let countries = {};
continents.map(continent => {
  countries[continent] = countriesJson[continent];
});
...
class EditProfileForm extends Component {
  ...
  renderCancelButton() {
    return <Button title={"Cancel"} onPress={this.props.goToProfile.bind(this)} />;
  }
  ...
  renderIfEboard() {
    ...
    if (this.checkPrivilege("eboard")) {
      return (
        <View>
          <Input placeholder="First Name" value={firstName} onChangeText={text => firstNameChanged(text)} />
          <Input placeholder="Last Name" value={lastName} onChangeText={text => lastNameChanged(text)} />
          <Input
          ...
          <DatePicker placeholder={"Birthday"} value={birthday} onSelect={text => birthDateChanged(text)} />
        </View>
      );
    }
  }
