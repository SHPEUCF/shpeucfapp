...
import { Input, Button, Spinner } from "../general";
import { RkAvoidKeyboard, RkPicker } from "react-native-ui-kitten";
...
class EditProfileForm extends Component {
  state = {
    collegeSelected: collegeNames.slice(0, 1),
  ...
  onButtonPress() {
    const { firstName, lastName, email, college, points, major, editMember, quote } = this.props;
  ...
  renderConfirmButton() {
    return <Button title={"Confirm"} onPress={this.onButtonPress.bind(this)} />;
  }

  renderCancelButton() {
    const { goBack } = this.props.navigation;
    return <Button title={"Cancel"} onPress={() => goBack()} />;
  }
  ...
  populateMajorArray(cName) {
    ...
    let temp = data.slice(cName[0].key - 1, cName[0].key);
          ...
          <ScrollView
            ...
            <RkAvoidKeyboard>
              ...
              <View style={styles.pickerTextInput}>
                <Input style={{ flex: 1 }} editable={false} value={this.state.collegeSelected[0].value} />
                <TouchableOpacity style={{ alignItems: "flex-end", margin: 10 }} onPress={this.showPicker1}>
              ...
              <View style={styles.pickerTextInput}>
                <Input style={{ flex: 1 }} editable={false} value={this.state.majorSelected[0]} />
                <TouchableOpacity style={{ alignItems: "flex-end", margin: 10 }} onPress={this.showPicker2}>
                  <Ionicons name={iconName} size={45} />
                </TouchableOpacity>
              </View>
